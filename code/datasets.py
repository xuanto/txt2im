...
def prepare_data(data, cuda_id):
    ...
    for i in range(len(imgs)):
        ...
        real_imgs.append((imgs[i]).cuda(cuda_id))
    ...
    captions = captions.cuda(cuda_id)
    sorted_cap_lens = sorted_cap_lens.cuda(cuda_id)
    return [real_imgs, captions, sorted_cap_lens,class_ids, keys]


def get_imgs(img_path, imsize, bbox=None, ts= None, transform=None, normalize=None):
    ...
    # re_img = transforms.Scale(256)(img)
    # ret.append(normalize(re_img))
    # return ret

    img = ts(img)
    return [img]
    # return transforms.Scale(256)(img)
...
class TextDataset(data.Dataset):
    ...
    def load_text_data(self, data_dir, split):
        # data_dir = "../data/birds" ; split = "../data/birds/test"
        filepath = os.path.join(data_dir, 'captions.pickle')
        train_names = self.load_filenames(data_dir, 'train')
        test_names = self.load_filenames(data_dir, 'test')
        if not os.path.isfile(filepath):
            print("no caption.pickle file !!!!!")
    ...
    def __getitem__(self, index):
        ...
        imgs = get_imgs(img_name, self.imsize, bbox, self.target_transform, self.transform, normalize=self.norm)
